...
function generateSlipHTML(emp, monthName, year, logoBase64) {
  ...
  const totalEarnings = Math.round(
    (emp.basicSalary || 0) + (emp.allowances || 0),
  );
  ...
  const joinDateFmt = emp.joinDate
    ? new Date(emp.joinDate).toLocaleDateString("en-PK", {
        day: "2-digit",
        month: "short",
        year: "numeric",
      })
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-gray-400">
            Total:{" "}
            <strong className="text-white font-bold">PKR {fmt(deptNet)}</strong>
          </span>
          <Button
            size="sm"
            variant="ghost"
            ...
            onClick={(e) => {
              e.stopPropagation();
              printMultipleSlips(employees, monthName, year);
            }}
          >
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                <th className="text-left px-4 py-2.5 font-semibold text-gray-500 w-48">
                  Employee
                </th>
                <th className="text-right px-3 py-2.5 font-semibold text-gray-500">
                  Basic Salary
                </th>
                <th className="text-center px-3 py-2.5 font-semibold text-gray-500">
                  Allowances
                </th>
                <th className="text-center px-3 py-2.5 font-semibold text-gray-500">
                  Deductions
                ...
                <th className="text-right px-3 py-2.5 font-semibold text-green-600">
                  Net Salary
                </th>
            ...
            <tbody className="divide-y divide-gray-100">
              {employees.map((emp) => {
                ...
                return (
                  <tr
                    key={emp._id}
                    className={`hover:bg-gray-50 transition-colors ${isSaved ? "bg-green-50/40" : ""}`}
                  >
                    ...
                    <td className="px-3 py-2.5 text-center">
                      <EditableCell
                        ...
                        onChange={(v) => onChange(emp._id, "allowances", v)}
                      />
                    </td>
                    <td className="px-3 py-2.5 text-center">
                      <EditableCell
                        value={emp.deductions}
                        onChange={(v) => onChange(emp._id, "deductions", v)}
                      />
                    </td>
                    <td className="px-3 py-2.5 text-center">
                      <EditableCell
                        value={emp.workingDays}
                        onChange={(v) => onChange(emp._id, "workingDays", v)}
                        min={1}
                      />
                    </td>
                    <td className="px-3 py-2.5 text-center">
                      <EditableCell
                        ...
                        onChange={(v) => onChange(emp._id, "presentDays", v)}
                      />
                    </td>
                    <td className="px-3 py-2.5 text-center">
                      <EditableCell
                        value={emp.leaves}
                        onChange={(v) => onChange(emp._id, "leaves", v)}
                      />
                    </td>
                    <td className="px-3 py-2.5 text-center">
                      <span
                        className={`font-bold tabular-nums ${absent > 0 ? "text-red-500" : "text-gray-300"}`}
                      >
                        {absent}
                      </span>
                    </td>
                    <td className="px-3 py-2.5 text-right">
                      <span
                        className={`font-bold tabular-nums ${net >= 0 ? "text-green-600" : "text-red-500"}`}
                      >
                        PKR {fmt(net)}
                      </span>
                    </td>
...
export default function PayrollPage() {
  ...
  const loadEmployees = useCallback(async () => {
    ...
    try {
      const [empRes, recordRes] = await Promise.all([
        ...
        axios
          .get(`/api/hr/payroll?month=${month}&year=${year}`)
          .catch(() => ({ data: [] })),
      ]);
  ...
  const handleSave = async (finalize = false) => {
    ...
    try {
      ...
      toast({
        title: finalize ? "Payroll finalized!" : "Payroll saved as draft",
        description: `${records.length} records stored for ${MONTHS[month - 1]} ${year}`,
      });
    } catch {
  ...
  return (
    <>
      <Header
        title="Payroll"
        subtitle="Manage monthly payroll, attendance and generate salary slips"
      />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              },
              {
                ...
                icon:
                  savedCount === filtered.length && filtered.length > 0
                    ? CheckCircle2
                    : FileText,
                color:
                  savedCount === filtered.length && filtered.length > 0
        ...
        {!loaded && (
          <div className="text-center py-24 text-gray-400">
            ...
            <p className="text-sm font-medium text-gray-500">
              Select month &amp; year, then click{" "}
              <strong>Load Employees</strong>
            </p>
            <p className="text-xs mt-1">
              Salary, allowances &amp; deductions will be pre-filled from
              employee records
            </p>
          </div>
        )}
          ...
          <div className="flex items-center justify-between bg-gray-900 text-white rounded-xl px-6 py-4">
            <div className="flex items-center gap-3">
              ...
              <span className="font-semibold">
                Grand Total Payroll — {monthName} {year}
              </span>
            </div>
