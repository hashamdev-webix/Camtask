    ...
    <div className="flex items-center gap-0.5">
      {show.map((a, i) => (
        <div
          key={i}
          title={a.name}
          className="w-7 h-7 rounded-full bg-gray-200 flex items-center justify-center text-xs font-bold text-gray-600 border-2 border-white -ml-1 first:ml-0"
        >
          {a.name?.charAt(0).toUpperCase()}
        </div>
      ))}
      {rest > 0 && (
        <div className="w-7 h-7 rounded-full bg-gray-700 flex items-center justify-center text-xs font-bold text-white border-2 border-white -ml-1">
          +{rest}
        </div>
      )}
    </div>
...
function ToolForm({ categories, users, onSuccess, onClose, editTool = null }) {
  ...
  const [form, setForm] = useState(
    editTool
      ? {
          ...
          primary_owner:
            editTool.primary_owner?._id || editTool.primary_owner || "",
          purchase_date: editTool.purchase_date
            ? new Date(editTool.purchase_date).toISOString().split("T")[0]
            : "",
          expiry_date: editTool.expiry_date
            ? new Date(editTool.expiry_date).toISOString().split("T")[0]
            : "",
          password: "",
        }
      : emptyForm,
  ...
  useEffect(() => {
    if (isSmmCat) set("is_smm_panel", true);
  }, [isSmmCat]);
  ...
  const addAssignee = (u) => {
    setAssignments((prev) => [
      ...prev,
      {
        ...
        name: u.name,
        access_type: "",
        access_given_date: new Date().toISOString().split("T")[0],
      },
    ]);
    setUserSearch("");
    setShowUserDD(false);
  ...
  const setAssigneeField = (id, k, v) =>
    setAssignments((prev) =>
      prev.map((a) => (a.employee_id === id ? { ...a, [k]: v } : a)),
    );
  ...
  const SectionHeader = ({ label, color = "bg-gray-800" }) => (
    <div
      className={`${color} text-white text-xs font-semibold uppercase tracking-wider px-3 py-1.5 rounded`}
    >
      {label}
    </div>
  );
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      >
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1">
              <Label>
                Tool / Software Name <span className="text-red-500">*</span>
              </Label>
              <Input
                value={form.name}
            ...
            <div className="space-y-1">
              ...
              <Select
                value={form.license_type || "_none"}
                onValueChange={(v) =>
                  set("license_type", v === "_none" ? "" : v)
                }
              >
                <SelectTrigger>
        ...
        <div className="space-y-3">
          <SectionHeader
            label="B — Credentials (Secure Vault)"
            color="bg-red-700"
          />
          <p className="text-xs text-red-600 bg-red-50 border border-red-100 rounded p-2">
            Passwords are encrypted using AES-256. Only Admin can reveal them.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1">
              <Label>
            ...
            <div className="space-y-1">
              <Label>
                Password {!editTool && <span className="text-red-500">*</span>}
              </Label>
              <div className="relative">
                <Input
                  ...
                  placeholder={
                    editTool ? "Leave blank to keep current" : "Enter password"
                  }
                  required={!editTool}
                  className="pr-10"
                />
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                onChange={(e) => set("account_owner", e.target.value)}
                placeholder="CEO / Camtask"
              />
            </div>
            <div className="space-y-1 sm:col-span-2">
              <Label>Additional Login Notes</Label>
              <textarea
                className="w-full min-h-[50px] p-3 text-sm border rounded-md resize-none focus:outline-none focus:ring-2 focus:ring-ring"
                value={form.additional_login_notes}
                onChange={(e) => set("additional_login_notes", e.target.value)}
                placeholder="e.g. Use VPN before logging in..."
              />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1">
              <Label>
                Seller / Purchased From <span className="text-red-500">*</span>
              </Label>
              <Input
                value={form.seller_name}
                onChange={(e) => set("seller_name", e.target.value)}
                placeholder="Hostinger.com"
                required
              />
            </div>
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              <Label>
                Package / Plan Name <span className="text-red-500">*</span>
              </Label>
              <Input
            ...
            <div className="space-y-1">
              ...
              <Input
                type="number"
                min="1"
                value={form.plan_duration_days}
                onChange={(e) => set("plan_duration_days", e.target.value)}
                placeholder="365"
                required
              />
            </div>
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={(v) => set("primary_owner", v)}
              >
                <SelectTrigger>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Select
                  value={form.smm_panel_type || "_none"}
                  onValueChange={(v) =>
                    set("smm_panel_type", v === "_none" ? "" : v)
                  }
                >
                  <SelectTrigger>
              ...
              <div className="space-y-1">
                ...
                <Input
                  ...
                  min="0"
                  step="0.01"
                  value={form.smm_current_balance}
                  onChange={(e) => set("smm_current_balance", e.target.value)}
                  placeholder="0"
                />
              </div>
              <div className="space-y-1">
                ...
                <Input
                  ...
                  min="0"
                  step="0.01"
                  value={form.smm_low_balance_threshold}
                  onChange={(e) =>
                    set("smm_low_balance_threshold", e.target.value)
                  }
                  placeholder="5000"
                />
              </div>
...
function ToolDetailDrawer({
  ...
  const fetchAssignments = async () => {
    try {
      const r = await axios.get(`/api/tools/${tool._id}/assignments`);
      setAssignments(r.data);
    } catch {}
  };
  const fetchRenewals = async () => {
    try {
      const r = await axios.get(`/api/tools/${tool._id}/renew`);
      setRenewals(r.data);
    } catch {}
  };
  const fetchTopups = async () => {
    try {
      const r = await axios.get(`/api/tools/${tool._id}/topups`);
      setTopups(r.data);
    } catch {}
  };
  ...
  const submitAssign = async () => {
    if (!addAssign.employee_id)
      return toast({ title: "Select an employee", variant: "destructive" });
    try {
  ...
  const expiry = new Date(tool.expiry_date);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      >
        ...
        {/* Expiry bar */}
        <div
          className={`mt-3 flex items-center gap-2 text-sm ${getExpiryStyle(daysLeft, tool.status)}`}
        >
          ...
          <div className="space-y-4">
            <div>
              ...
              <InfoRow
                label="Visibility"
                value={
                  VISIBILITY_OPTIONS.find((o) => o.value === tool.visibility)
                    ?.label
                }
              />
            </div>
            <div>
              ...
              <InfoRow
                label="Purchase Date"
                value={formatDate(tool.purchase_date)}
              />
              <InfoRow
                label="Price Paid"
                value={tool.price?.toLocaleString()}
              />
              <InfoRow label="Billing Cycle" value={tool.billing_cycle} />
              <InfoRow label="Plan Name" value={tool.plan_name} />
              <InfoRow
                label="Duration"
                value={`${tool.plan_duration_days} days`}
              />
              <InfoRow
                label="Auto-Renewal"
                value={tool.auto_renewal ? "Enabled" : "Disabled"}
              />
              <InfoRow
                label="Monthly Est."
                value={`${Math.round((tool.price / tool.plan_duration_days) * 30).toLocaleString()}`}
              />
            </div>
            <div>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">
            ...
            {tool.is_smm_panel && (
              <div>
                ...
                <InfoRow
                  ...
                  value={tool.smm_current_balance?.toLocaleString()}
                />
                <InfoRow
                  label="Low Balance Alert"
                  value={tool.smm_low_balance_threshold?.toLocaleString()}
                />
              </div>
            )}
          </div>
        ...
        {tab === "credentials" && (
          <div className="space-y-4">
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-xs text-amber-700">
              All credential views are permanently logged. Password auto-hides
              after 30 seconds.
            </div>
            <div className="space-y-3">
              ...
              <InfoRow
                ...
                value={tool.additional_login_notes}
              />
            </div>
            {isAdmin && (
              <div className="pt-2">
                ...
                {password ? (
                  <div className="flex items-center gap-2">
                    <div className="flex-1 bg-gray-900 text-green-400 font-mono text-sm px-3 py-2 rounded-md">
                      {password}
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 bg-gray-100 text-gray-400 px-3 py-2 rounded-md text-sm">
                      ••••••••••••
                    </div>
                    <Button
                      size="sm"
          ...
          <div className="space-y-4">
            {isAdmin && (
              <div className="bg-gray-50 rounded-lg p-3 space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  <Select
                    value={addAssign.employee_id}
                    onValueChange={(v) =>
                      setAddAssign((a) => ({ ...a, employee_id: v }))
                    }
                  >
                  ...
                  <Select
                    value={addAssign.access_type || "_none"}
                    onValueChange={(v) =>
                      setAddAssign((a) => ({
                        ...a,
                        access_type: v === "_none" ? "" : v,
                      }))
                    }
                  >
                    <SelectTrigger className="h-8 text-xs">
                  ...
                  <Input
                    ...
                    value={addAssign.access_given_date}
                    onChange={(e) =>
                      setAddAssign((a) => ({
                        ...a,
                        access_given_date: e.target.value,
                      }))
                    }
                  />
                </div>
            ...
            <div className="space-y-2">
              ...
              {assignments.map((a) => (
                <div
                  key={a._id}
                  className={`flex items-center justify-between p-3 rounded-lg border ${a.status === "revoked" ? "bg-gray-50 opacity-60" : "bg-white"}`}
                >
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-full bg-red-100 flex items-center justify-center text-xs font-bold text-red-700">
                      {a.employee_id
                        ? `${a.employee_id.firstName || ""}${a.employee_id.lastName || ""}`
                            .charAt(0)
                            .toUpperCase()
                        : "?"}
                    </div>
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        {a.employee_id?.designation ||
                          a.access_type ||
                          "No type set"}{" "}
                        · Given {formatDate(a.access_given_date)}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span
                      className={`text-xs font-medium px-2 py-0.5 rounded-full ${a.status === "active" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
                    >
                      {a.status === "active"
                        ? "Active"
                        : `Revoked ${formatDate(a.access_revoked_date)}`}
                    </span>
                    {isAdmin && a.status === "active" && (
                      <Button
                        ...
                        onClick={() => revokeAssignment(a._id)}
                      >
                        Revoke
                      </Button>
                    )}
          ...
          <div className="space-y-4">
            ...
            {showRenewForm && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  <div className="space-y-1">
                    <Label className="text-xs">New Purchase Date</Label>
                    <Input
                      type="date"
                      value={renewForm.purchase_date}
                      onChange={(e) =>
                        setRenewForm((f) => ({
                          ...f,
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      type="number"
                      value={renewForm.plan_duration_days}
                      onChange={(e) =>
                        setRenewForm((f) => ({
                          ...f,
                          plan_duration_days: e.target.value,
                        }))
                      }
                      placeholder="365"
                      className="h-8 text-xs"
              ...
              {renewals.map((r, i) => (
                ...
                >
                  <div>
                    <p className="text-sm font-medium">
                      #{r.renewal_number} —{" "}
                      {i === 0 ? "Initial Purchase" : "Renewal"}
                    </p>
                    <p className="text-xs text-gray-400">
                      {formatDate(r.purchase_date)} →{" "}
                      {formatDate(r.expiry_date)} · by {r.renewed_by?.name}
                    </p>
                  </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {tool.smm_current_balance?.toLocaleString() || "0"}
                </p>
              </div>
              {isAdmin && (
                <Button
                  size="sm"
                  onClick={() => setShowTopupForm((p) => !p)}
                  className="bg-orange-600 hover:bg-orange-700 text-white"
                >
                  <Plus className="h-3.5 w-3.5 mr-1" /> Add Top-up
                </Button>
              )}
            </div>
            {showTopupForm && (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      placeholder="Bank Transfer / JazzCash"
                      className="h-8 text-xs"
                    />
                  </div>
                  <div className="space-y-1">
            ...
            <div className="space-y-2">
              ...
              {topups.map((t, i) => (
                ...
                >
                  <div>
                    <p className="text-sm font-medium">
                      #{topups.length - i} · {formatDate(t.topup_date)}
                    </p>
                    <p className="text-xs text-gray-400">
                      {t.payment_method || "No method"} · {t.added_by?.name}{" "}
                      {t.notes ? `· ${t.notes}` : ""}
                    </p>
                  </div>
  ...
  if (!stats)
    return (
      <div className="text-center text-gray-400 py-12">
        Dashboard unavailable (Admin only)
      </div>
    );
                ...
                <PieChart>
                  ...
                  >
                    {stats.statusBreakdown.map((s, i) => {
                      ...
                      return (
                        <Cell key={i} fill={colors[s.status] || "#6366f1"} />
                      );
                    })}
                  </Pie>
                  <Tooltip />
...
export default function ToolsPage() {
  ...
  return (
    <div className="flex h-full overflow-hidden">
      {/* Main Content */}
      <div
        className={`flex flex-col flex-1 overflow-hidden transition-all ${selectedTool ? "mr-[480px]" : ""}`}
      >
        <Header
          title="Tools & Access Management"
          subtitle="Track all software subscriptions, credentials, and team access"
        />
        <div className="flex-1 overflow-auto p-4 md:p-6 space-y-4">
          ...
          <div className="flex items-center justify-between border-b">
            <div className="flex">
              {[
                { id: "directory", label: "Tool Directory" },
          ...
          {activeTab === "dashboard" ? (
            ...
            <>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Select
                  value={catFilter}
                  onValueChange={(v) => setCatFilter(v === "all" ? "" : v)}
                >
                    ...
                    <tbody className="divide-y">
                      {loading ? (
                        ...
                        <tr>
                          ...
                          >
                            No tools found.{" "}
                            {isAdmin && "Add your first tool above."}
                          </td>
                        </tr>
                      ) : (
                        tools.map((tool) => {
                          ...
                          return (
                            <tr
                              key={tool._id}
                              className={`cursor-pointer transition-colors hover:bg-gray-50 ${getRowBg(tool.daysLeft, tool.status)}`}
                              onClick={() => {
                            ...
                            >
                              ...
                              <td className="px-4 py-3 hidden md:table-cell">
                                {tool.assignees?.length > 0 ? (
                                  <AvatarChips assignees={tool.assignees} />
                                ) : (
                              ...
                              <td className="px-4 py-3">
                                <p
                                  className={`text-xs ${getExpiryStyle(tool.daysLeft, tool.status)}`}
                                >
                                  {formatDate(tool.expiry_date)}
                                </p>
                                {tool.status === "active" && (
                                  <p
                                    className={`text-xs ${getExpiryStyle(tool.daysLeft, tool.status)}`}
                                  >
                                    {tool.daysLeft > 0
                                      ? `${tool.daysLeft}d left`
                                      : "Expired"}
                                  </p>
                                )}
                              </td>
                              <td className="px-4 py-3 hidden lg:table-cell text-gray-600">
                                {tool.monthlyEst?.toLocaleString()}
                              </td>
                              <td className="px-4 py-3 hidden lg:table-cell text-gray-600">
                                {tool.primary_owner?.name || "—"}
                              </td>
                              <td
                                className="px-4 py-3"
                                ...
                                <div className="flex items-center gap-1">
                                  ...
                                  {isAdmin && (
                                    <Button
                                      size="sm"
                                      variant="ghost"
                                      className="h-7 px-2 text-red-300 hover:text-red-600"
