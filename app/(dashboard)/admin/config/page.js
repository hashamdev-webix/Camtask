...
function AssignModal({ item, entity, users, onClose, onSaved }) {
  const [selected, setSelected] = useState(
    () => new Set((item.assigned_users || []).map((u) => u._id || u)),
  );
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <p className="text-xs text-gray-400 mt-1">
            {item.account_name || item.name || item.label || "This item"} ·{" "}
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition-colors ${
                    checked
                      ? "bg-red-50 border border-indigo-200"
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {entity === "platforms" && (
        <>
          ...
          <div className="space-y-1">
            <Label>Icon Slug (optional)</Label>
            <Input
        ...
        <>
          <div className="space-y-1">
            ...
            <Select
              ...
              onValueChange={(v) => set("platform_id", v)}
            >
              <SelectTrigger>
...
export default function AdminConfigPage() {
  ...
  useEffect(() => {
    axios
      .get("/api/admin/config/platforms")
      .then((r) => setPlatforms(r.data))
      .catch(() => {});
    axios
      .get("/api/users")
      .then((r) => setUsers(r.data.filter((u) => u.isActive)))
      .catch(() => {});
  }, []);

  const handleToggle = async (item) => {
    try {
      await axios.patch(`/api/admin/config/${activeTab}`, {
        id: item._id,
        is_active: !item.is_active,
      });
      setItems((prev) =>
        prev.map((i) =>
          i._id === item._id ? { ...i, is_active: !i.is_active } : i,
        ),
      );
    } catch {
  ...
  return (
    <>
      <Header
        ...
        subtitle="Manage master data for platforms, niches, and outreach"
      />
