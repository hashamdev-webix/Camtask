...
export default function LeadDetailDrawer({
  ...
  const title =
    leadType === "social" ? lead.customer_id_url : lead.business_name;
        ...
        >
          <TabsList className="flex border-b px-4 flex-shrink-0 gap-1">
            {[
              ...
              "remarks",
              "timeline",
              ...(outreachPanel ? ["outreach"] : []),
            ].map((tab) => (
              <TabsTrigger
                key={tab}
                value={tab}
                className="px-3 py-2.5 text-sm font-medium capitalize border-b-2 border-transparent data-[state=active]:border-indigo-600 data-[state=active]:text-indigo-600 text-gray-500 hover:text-gray-900 transition-colors"
              >
                {tab}
              </TabsTrigger>
            ))}
                ...
                <div className="grid grid-cols-2 gap-3">
                  {leadType === "social" ? (
                    <>
                      ...
                      <Field
                        label="ID Account"
                        value={lead.social_account_id?.account_name}
                      />
                      ...
                      <div className="col-span-2">
                        <Field
                          label="Customer URL"
                          value={lead.customer_id_url}
                          link={lead.customer_id_url}
                        />
                      </div>
                    </>
                  ) : (
                  ...
                  <Field
                    label="Owner"
                    value={lead.owner_user_id?.name || "—"}
                  />
                  {lead.assigned_to && (
                    <Field label="Assigned To" value={lead.assigned_to?.name} />
                  )}
                </div>
              </div>
            </TabsContent>
